...
public class TypesafeConfigUtils {
    ...
    public static Optional<Config> resolveCustomConfig(Path appDataDir) {
        ...
        Path customConfigPath = appDataDir.resolve(configName);
        if (Files.exists(customConfigPath)) {
            try {
                com.typesafe.config.Config config = ConfigFactory.parseFile(customConfigPath.toFile());
                config.checkValid(ConfigFactory.defaultReference(), "application");
                log.info("Using custom config file: {}", customConfigPath.toAbsolutePath());
                return Optional.of(config);
            } catch (Exception e) {
                log.error("Could not load custom config file {}", customConfigPath.toAbsolutePath(), e);
            }
        }
