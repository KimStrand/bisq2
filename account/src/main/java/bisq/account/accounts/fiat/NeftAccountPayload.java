...
public final class NeftAccountPayload extends CountryBasedAccountPayload implements SingleCurrencyAccountPayload {
    ...
    public NeftAccountPayload(String id, String holderName, String accountNr, String ifsc) {
        this(id, AccountUtils.generateSalt(), holderName, accountNr, ifsc);
    }

    public NeftAccountPayload(String id,
                              byte[] salt,
                              String holderName,
                              String accountNr,
                              String ifsc) {
    ...
    protected bisq.account.protobuf.CountryBasedAccountPayload.Builder getCountryBasedAccountPayloadBuilder(boolean serializeForHash) {
        return super.getCountryBasedAccountPayloadBuilder(serializeForHash)
                .setNeftAccountPayload(toNeftAccountPayloadProto(serializeForHash));
    }

    private bisq.account.protobuf.NeftAccountPayload toNeftAccountPayloadProto(boolean serializeForHash) {
        return resolveBuilder(getNeftAccountPayloadBuilder(serializeForHash), serializeForHash).build();
    }

    private bisq.account.protobuf.NeftAccountPayload.Builder getNeftAccountPayloadBuilder(boolean serializeForHash) {
        return bisq.account.protobuf.NeftAccountPayload.newBuilder()
                .setHolderName(holderName)
                .setAccountNr(accountNr)
                .setIfsc(ifsc);
    }

    public static NeftAccountPayload fromProto(bisq.account.protobuf.AccountPayload proto) {
        var payload = proto.getCountryBasedAccountPayload().getNeftAccountPayload();
        return new NeftAccountPayload(
                proto.getId(),
    ...
    public FiatPaymentMethod getPaymentMethod() {
        return FiatPaymentMethod.fromPaymentRail(FiatPaymentRail.NEFT);
    }
